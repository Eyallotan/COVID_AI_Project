...
if __name__ == "__main__":
    ...
    # plot_time_series(time_series, 'Daily cases Tel Aviv', 'New Cases')
    ...
    # transformer = DataTransformation(rolling_average_series)
    # transformer.sqrt()
    # time_series_diff = transformer.difference(1)
    # plot_time_series(time_series_diff)

    s_test = StationarityTests()
    s_test.ADF_Stationarity_Test(national_daily_cases, printResults=True)
    print("Is the time series stationary? {0}".format(s_test.is_stationary))

    # Get training and test sets
    train_end = datetime(2021, 8, 20)
    test_end = datetime(2021, 9, 20)
    runner = AlgoRunner(rolling_average_series, train_end, test_end)
    # runner.plot_correlation_plots()
    #runner.run_ar_regressor(7, use_rolling_forecast=True)
    #runner.run_ma_regressor(7, use_rolling_forecast=True)
    runner.run_arma_regressor(7, 7, use_rolling_forecast=False)
    runner.run_arma_regressor(7, 7, use_rolling_forecast=True)
